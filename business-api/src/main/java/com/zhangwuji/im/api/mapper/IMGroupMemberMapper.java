...
public interface IMGroupMemberMapper extends BaseMapper<IMGroupMember> {
    ...
    @Select("select a.id as groupId,u.id,u.id as peerId,u.username,u.nickname,u.realname,u.sex,u.avatar,u.domain,u.phone,u.email,u.sign_info,u.updated,u.created from  on_IMGroupMember a LEFT JOIN on_IMUser u ON a.userId=u.id  where  a.status=0 and a.groupId=#{id}")
