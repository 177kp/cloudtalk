...
void CHttpQuery::DispatchQuery(std::string& url, std::string& post_data, CHttpConn* pHttpConn)
{
    ...
    else if (strcmp(url.c_str(), "/api/addrobot") == 0)
    {
        _ApiAddRobot(strAppKey, value, pHttpConn);
    }
    else {
        log("url not support ");
        pHttpConn->Close();
...
void CHttpQuery::_ApiAddRobot(const string& strAppKey, Json::Value &post_json_obj, CHttpConn *pHttpConn) {
    ...
    uint32_t userid=post_json_obj["userid"].asInt();
    CImUser* pImUser = CImUserManager::GetInstance()->GetImUserById(userid);
    if (!pImUser) {
        pImUser = new CImUser(to_string(userid));
        CImUserManager::GetInstance()->AddImUserById(userid, pImUser);
    }
...
void CHttpQuery::_ApiSendMsg(const string& strAppKey, Json::Value &post_json_obj, CHttpConn *pHttpConn) {
    ...
    if (post_json_obj["to_session_id"].isNull() || post_json_obj["msg_type"].isNull() || post_json_obj["session_type"].isNull() || post_json_obj["msg_data"].isNull() || post_json_obj["from_user_id"].isNull()) {
        log("send msg 参数不完整.");
        char *response_buf = PackSendResult(HTTP_ERROR_PARMENT, HTTP_ERROR_MSG[1].c_str());
        pHttpConn->Send(response_buf, (uint32_t) strlen(response_buf));
        pHttpConn->Close();
    ...
    */
    ...
    msg.set_session_type((IM::BaseDefine::SessionType)post_json_obj["session_type"].asInt());
