...
public class LoginActivity extends TTBaseActivity {
    ...
    public static LoginActivity ins=null;
    ...
    public void outLogin(com.alibaba.fastjson.JSONObject jsonObject)
    {
        ...
        if (code == 200) {
            ...
            com.alibaba.fastjson.JSONObject bqmmplus = data.getJSONObject("bqmmplugin");
            loginInfoSp.setBqmmPlug(bqmmplus.getString("appid"), bqmmplus.getString("appsecret"));

            //验证成功后。保存登录相关的信息。
            loginInfoSp.setLoginInfo(userinfo.getPhone(), "", mToken, userinfo.getPeerId());
            imService.getLoginManager().login(UrlConstant.appid, userinfo.getPeerId(), userinfo.getPhone(), mToken);
        }
