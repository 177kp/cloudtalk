...
//                    log("userId=%u, peerId=%u, msgId=%u", nUserId, nPeerId, it->msg_id());
    ...
    void sendMessage(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            log("sendMessage fromId=%u, nToId=%u", nFromId, nToId);
            ...
            if (IM::BaseDefine::MsgType_IsValid(nMsgType))
            {
                if(nMsgLen != 0)
                {
                    ...
                    if(nSessionType==IM::BaseDefine::SESSION_TYPE_GROUP  && nMsgType == IM::BaseDefine::MSG_TYPE_TEXT) {
                    ...
                    } else if (nSessionType==IM::BaseDefine::SESSION_TYPE_GROUP  && nMsgType == IM::BaseDefine::MSG_TYPE_AUDIO) {
                        CGroupModel* pGroupModel = CGroupModel::getInstance();
                        if (pGroupModel->isValidateGroupId(nToId)&& pGroupModel->isInGroup(nFromId, nToId))
                        {
                            nSessionId = CSessionModel::getInstance()->getSessionId(nFromId, nToId, IM::BaseDefine::SESSION_TYPE_GROUP, false);
                    ...
                    } else if(nSessionType==IM::BaseDefine::SESSION_TYPE_SINGLE && nMsgType== IM::BaseDefine::MSG_TYPE_TEXT) {
                        if (nFromId != nToId) {
                    ...
                    } else if(nSessionType==IM::BaseDefine::SESSION_TYPE_SINGLE && nMsgType == IM::BaseDefine::MSG_TYPE_AUDIO) {
