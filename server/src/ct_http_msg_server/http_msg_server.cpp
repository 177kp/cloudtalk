...
void websocket_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
	if (msg == NETLIB_MSG_CONNECT)
	{
		CWebSocketConn* pConn = new CWebSocketConn();
		pConn->OnConnect(handle);
	}
	else
	{
...
int main(int argc, char* argv[])
{
	if ((argc == 2) && (strcmp(argv[1], "-v") == 0)) {
		printf("Server Version: CTHttpMsgServer/%s\n", VERSION);
	...
	log("*****************************************");
	log("cloudtalk ct_http_msg_server服务启动中....");
	log("本模块提供Http api接口服务与 websocket 服务");
	log("本模块可提供分布式部署,同时支持http与ws协议");
	log("*****************************************");
	log("MsgServer max files can open: %d ", getdtablesize());

    
	CConfigFileReader config_file("ct_httpmsgserver.conf");
	...
	printf("cloudtalk ct_http_msg_server start listen on: %s:%d\n", listen_ip, listen_port);
	...
	for (uint32_t i = 0; i < ws_listen_ip_list.GetItemCnt(); i++) {
		ret = netlib_listen(ws_listen_ip_list.GetItem(i), listen_ws_port, websocket_callback, NULL);
		if (ret == NETLIB_ERROR)
			return ret;
	}
	init_websocket_conn();
    printf("cloudtalk websocket_server start listen on: %s:%d\n", websocket_listen_ip, listen_ws_port);
	printf("服务启动成功,件事监听中...\n");
