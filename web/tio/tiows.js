...
tio.ws = function (ws_protocol, ip, port, paramStr, param, handler, heartbeatTimeout, reconnInterval, binaryType) {
  ...
  this.connect = function (isReconnect) {
    ...
    ws.onopen = function (event) {
	  self.islogin=self.handler.getloginstat.call(self.handler);
	  if(self.islogin) 
      {  //如果之前已经登录过,重新登录服务器
	     self.ws.send(self.logindata);
      }
  ...
  this.ping = function () {
	this.islogin=this.handler.isUserLogin(); //获取是否登录成功
  ...
  this.loginServer = function(){
    this.ws.send(this.logindata);
  };
