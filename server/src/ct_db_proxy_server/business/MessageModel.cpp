                               ...
                               uint32_t nMsgCnt, list<IM::BaseDefine::MsgInfo>& lsMsg)
{
	...
	if (nRelateId != INVALID_VALUE)
    {
        ...
        if (pDBConn)
        {
            ...
            string strTableName = "on_IMMessage";
            string strSql;
            if (nMsgId == 0) {
                strSql = "select * from " + strTableName + " force index (idx_relateId_status_created) where relateId= " + int2string(nRelateId) + " and (fromId="+int2string(nUserId)+" or toId="+int2string(nUserId)+") and status = 0 order by created desc, id desc limit " + int2string(nMsgCnt);
            }
            else
            {
                strSql = "select * from " + strTableName + " force index (idx_relateId_status_created) where relateId= " + int2string(nRelateId) + "  and (fromId="+int2string(nUserId)+" or toId="+int2string(nUserId)+")  and status = 0 and msgId <=" + int2string(nMsgId)+ " order by created desc, id desc limit " + int2string(nMsgCnt);
            }
            log("getMessage: %s", strSql.c_str());
            ...
            if (pResultSet)
            {
                while (pResultSet->Next())
                {
                    ...
                    cMsg.set_session_type(IM::BaseDefine::SESSION_TYPE_SINGLE);
...
void CMessageModel::getLastMsg(uint32_t nFromId, uint32_t nToId, uint32_t& nMsgId, string& strMsgData, IM::BaseDefine::MsgType& nMsgType, uint32_t nStatus)
{
    ...
    if (nRelateId != INVALID_VALUE)
    {
        ...
        if (pDBConn)
        {
            //string strTableName = "on_IMMessage_" + int2string(nRelateId % 8);
            string strTableName = "on_IMMessage";
            string strSql = "select msgId,type,content from " + strTableName + " force index (idx_relateId_status_created) where relateId= " + int2string(nRelateId) + "  and (fromId="+int2string(nFromId)+" or toId="+int2string(nFromId)+")  and type<100 and status = 0 order by created desc, id desc limit 1";
            CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
            if (pResultSet)
            {
                while (pResultSet->Next())
...
void CMessageModel::getMsgByMsgId(uint32_t nUserId, uint32_t nPeerId, const list<uint32_t> &lsMsgId, list<IM::BaseDefine::MsgInfo> &lsMsg)
{
    ...
    if (pDBConn)
    {
        ...
        string strSql = "select * from " + strTableName + " where relateId=" + int2string(nRelateId) + "  and status=0 and msgId in (" + strClause + ")  and  (fromId="+int2string(nUserId)+" or toId="+int2string(nUserId)+")  order by created desc, id desc limit 100";
        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
        if (pResultSet)
        {
            while (pResultSet->Next())
