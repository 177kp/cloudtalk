...
msgResp.set_user_id(from_user_id);
   ...
   pUser->set_user_id(it->user_id());
    ...
    //更新用户在线状态
    void doUpdateUserOnline(CImPdu* pPdu, uint32_t conn_uuid) {
        IM::Server::IMUserStatusUpdate msg;
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
    	      uint32_t user_status = msg.user_status();
	          uint32_t user_id = msg.user_id();
              bool result = CUserModel::getInstance()->updateUserOnline(user_id, user_status);
        } else {
            log("doUpdateUserOnline: IMUserStatusUpdate ParseFromArray failed!!!");
        }
    }
