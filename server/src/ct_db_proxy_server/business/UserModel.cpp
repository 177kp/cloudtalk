...
bool CUserModel::updateUserOnline(uint32_t nUserId,uint32_t status)
{
    bool bRet = false;
    CDBManager* pDBManager = CDBManager::getInstance();
    ...
    if (pDBConn)
    {
        ...
        string strSql = "update on_IMOnline set `status`="+int2string(status)+",`updatetime`=" + int2string(nNow) + " where uid=" + int2string(nUserId);
        bRet = pDBConn->ExecuteUpdate(strSql.c_str());
        pDBManager->RelDBConn(pDBConn);
    }
    else
    {
        log("no db connection for cloudtalk");
    }
    return bRet;
}
