...
public class MessageActivity extends TTBaseActivity
    ...
    private IMServiceConnector imServiceConnector = new IMServiceConnector() {
        ...
        public void onIMServiceConnected() {
            ...
            loginInfoSp.init(MessageActivity.this);
            LoginInfoSp.bqmmplugSpIdentity bqmmplugSpIdentity=loginInfoSp.getbqmmplug();

            DongtuStore.initConfig(getBaseContext(), bqmmplugSpIdentity.getAppid(), bqmmplugSpIdentity.getSecret());
            DongtuStore.setUserInfo(imService.getLoginManager().getLoginId()+"", imService.getLoginManager().getLoginInfo().getMainName(), DTGender.MALE, "", imService.getLoginManager().getLoginInfo().getEmail(), imService.getLoginManager().getLoginInfo().getPhone(), null);
    ...
    private void initView() {
       ...
       // setLeftText(getResources().getString(R.string.top_left_back));
        setRightButton(R.drawable.ct_user_info);
        ...
        messageEdt = (DTStoreEditView) this.findViewById(R.id.message_text);
        ...
        bqmmKeyboard = (DTStoreKeyboard) findViewById(R.id.bqmm_keyboard);
        bqmmSendButton = (Button) findViewById(R.id.btn_send);
        //EMO_LAYOUT
        emoLayout = (LinearLayout) findViewById(R.id.emo_layout);


        DongtuStore.setEditText(messageEdt);
        DongtuStore.setKeyboard(bqmmKeyboard);
        DongtuStore.setupSearchPopupAboveView(findViewById(R.id.tt_layout_bottom), messageEdt);

//        DongtuStore.setKeyboard(bqmmKeyboard, new IGifButtonClickListener() {
//            @Override
...
//        });
        ...
        messageEdt.setOnTouchListener((v, event) -> {
        ...
        DongtuStore.setSendMessageListener(new DTStoreSendMessageListener() {
            ...
            public void onSendSticker(final DTStoreSticker sticker) {
                ...
                bigMojiMessage.setEmotext(sticker.text);
                bigMojiMessage.setCode(sticker.code);
                bigMojiMessage.setDisplaytext(sticker.text); //为显示在会话窗口的txt
            ...
            public void onSendDTImage(final DTImage image) {
                ...
                bigMojiMessage.setThumbail(image.getImage());
                bigMojiMessage.setMainimage(image.getImage());
                bigMojiMessage.setEmotext(image.getText());
                bigMojiMessage.setCode(image.getId());
                bigMojiMessage.setDisplaytext(image.getText()); //为显示在会话窗口的txt
    ...
    private void setTitleByUser() {
        ...
        switch (peerType) {
            case DBConstant.SESSION_TYPE_GROUP: {
                group = (Group) peerEntity;
                Set<Integer> memberLists = group.getlistGroupMemberIds();
                setTitle(group.getMainName()+"("+memberLists.size()+")");
                ...
                imAction.getGroupInfo(group.getId() + "", new BaseAction.ResultCallback<String>() {
                    @Override
                    public void onSuccess(String s) {
                        ...
                        if(gdata.getIntValue("disable_send_msg")==1 || gdata.getIntValue("MeIsDisableSendMsg")==1) {
                           if(group.getCreatorId()!=imService.getLoginManager().getLoginId()) {
                               is_dis_send_msg = true; //禁言中。。。
        ...
        switch (event.event) {
            ...
            case GROUP_DIS_MESSAGE:
            {
                ...
                if (currentSessionKey.equals(entity.getSessionKey())) {
                    ...
                    if(group.getCreatorId()!=imService.getLoginManager().getLoginId()) {
                        if (jsonObject.getIntValue("disable_send_msg") == 1) {
                            is_dis_send_msg = true; //禁言中。。。
                            messageEdt.setEnabled(false);
                        } else {
                            is_dis_send_msg = false; //禁言中。。。
                            messageEdt.setEnabled(true);
                        }
                    }
                }
            }
...
//                mlist.setStackFromBottom(false);
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        if (s.length() > 0 && count>0) {
            ...
            char curstr=s.charAt(count-1);
            if(curstr=='@')
            {
                Intent intent = new Intent(this, GroupMemberListActivity.class);
                intent.putExtra(IntentConstant.KEY_SESSION_KEY, peerEntity.getSessionKey());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case 828:
            {
                Integer selectUID=data.getIntExtra("selectUID",0);
                if(selectUID!=0)
                {
                    User userinfo=imService.getContactManager().findContact(selectUID);
                    if(userinfo!=null)
                    {
                        AtUserList.add(userinfo);
                        messageEdt.append(userinfo.getMainName()+",");
                    }
    ...
    public void onClick(View v) {
        ...
        switch (id) {
            ...
            case R.id.send_message_btn: {
               ...
               // textMessage.setSessionType(IMBaseDefine.SessionType.SESSION_TYPE_SINGLE.getNumber());
                imService.getMessageManager().sendText(textMessage);
                messageEdt.setText("");
                pushList(textMessage);
                scrollToBottomListItem();
                ...
                {
                    for(User u:AtUserList)
                    {
                        JSONObject jsonObject=new JSONObject();
                        jsonObject.put("type","atme");
                        jsonObject.put("sessionKey",currentSessionKey);
                        jsonObject.put("fromName",loginUser.getMainName());
                        jsonObject.put("groupName",peerEntity.getMainName());
                        jsonObject.put("message",content);
                        SystemMessage systemMessage=SystemMessage.buildForSend(jsonObject.toJSONString(),loginUser,u);
                        imService.getMessageManager().sendSystemNoticeMessage(systemMessage);
                    }
                    AtUserList.clear();
                }
