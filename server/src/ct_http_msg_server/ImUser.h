...
public:
    ...
    void AddMsgConn(uint32_t handle, CWebSocketConn* pMsgConn) { m_conn_map[handle] = pMsgConn; }
    void DelMsgConn(uint32_t handle) { m_conn_map.erase(handle); }
    CWebSocketConn* GetMsgConn(uint32_t handle);
    void ValidateMsgConn(uint32_t handle, CWebSocketConn* pMsgConn);
    
    void AddUnValidateMsgConn(CWebSocketConn* pMsgConn) { m_unvalidate_conn_set.insert(pMsgConn); }
    void DelUnValidateMsgConn(CWebSocketConn* pMsgConn) { m_unvalidate_conn_set.erase(pMsgConn); }
    CWebSocketConn* GetUnValidateMsgConn(uint32_t handle);

    map<uint32_t, CWebSocketConn*>& GetMsgConnMap() { return m_conn_map; }

    void BroadcastPdu(CImPdu* pPdu, CWebSocketConn* pFromConn = NULL);
    void BroadcastPduWithOutMobile(CImPdu* pPdu, CWebSocketConn* pFromConn = NULL);
    void BroadcastPduToMobile(CImPdu* pPdu, CWebSocketConn* pFromConn = NULL);
    void BroadcastClientMsgData(CImPdu* pPdu, uint32_t msg_id, CWebSocketConn* pFromConn = NULL, uint32_t from_id = 0);
    void BroadcastData(void* buff, uint32_t len, CWebSocketConn* pFromConn = NULL);
        
    void HandleKickUser(CWebSocketConn* pConn, uint32_t reason);
    
    bool KickOutSameClientType(uint32_t client_type, uint32_t reason, CWebSocketConn* pFromConn = NULL);
...
private:
    ...
    map<uint32_t /* handle */, CWebSocketConn*>	m_conn_map;
    set<CWebSocketConn*> m_unvalidate_conn_set;
};
...
public:
    ...
    CWebSocketConn* GetMsgConnByHandle(uint32_t user_id, uint32_t handle);
