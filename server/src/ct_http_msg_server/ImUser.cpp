...
CWebSocketConn* CImUser::GetUnValidateMsgConn(uint32_t handle)
{
    for (set<CWebSocketConn*>::iterator it = m_unvalidate_conn_set.begin(); it != m_unvalidate_conn_set.end(); it++)
    {
        CWebSocketConn* pConn = *it;
...
CWebSocketConn* CImUser::GetMsgConn(uint32_t handle)
{
    CWebSocketConn* pMsgConn = NULL;
    map<uint32_t, CWebSocketConn*>::iterator it = m_conn_map.find(handle);
...
void CImUser::ValidateMsgConn(uint32_t handle, CWebSocketConn* pMsgConn)
{
...
{
    uint32_t conn_cnt = 0;
    for (map<uint32_t, CWebSocketConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
    {
        CWebSocketConn* pConn = it->second;
       // if (pConn->IsOpen()) {
       //     conn_cnt++;
       // }
    }
...
void CImUser::BroadcastPdu(CImPdu* pPdu, CWebSocketConn* pFromConn)
{
    for (map<uint32_t, CWebSocketConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
    {
        CWebSocketConn* pConn = it->second;
...
void CImUser::BroadcastPduWithOutMobile(CImPdu *pPdu, CWebSocketConn* pFromConn)
{
    for (map<uint32_t, CWebSocketConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
    {
        CWebSocketConn* pConn = it->second;
...
void CImUser::BroadcastPduToMobile(CImPdu* pPdu, CWebSocketConn* pFromConn)
{
    for (map<uint32_t, CWebSocketConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
    {
        CWebSocketConn* pConn = it->second;
...
void CImUser::BroadcastClientMsgData(CImPdu* pPdu, uint32_t msg_id, CWebSocketConn* pFromConn, uint32_t from_id)
{
    for (map<uint32_t, CWebSocketConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
    {
        CWebSocketConn* pConn = it->second;
...
void CImUser::BroadcastData(void *buff, uint32_t len, CWebSocketConn* pFromConn)
{
    if(!buff)
        return;
    for (map<uint32_t, CWebSocketConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
...
void CImUser::HandleKickUser(CWebSocketConn* pConn, uint32_t reason)
{
...
bool CImUser::KickOutSameClientType(uint32_t client_type, uint32_t reason, CWebSocketConn* pFromConn)
{
    for (map<uint32_t, CWebSocketConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
    {
        CWebSocketConn* pMsgConn = it->second;
...
uint32_t CImUser::GetClientTypeFlag()
{
    ...
    map<uint32_t, CWebSocketConn*>::iterator it = m_conn_map.begin();
...
CWebSocketConn* CImUserManager::GetMsgConnByHandle(uint32_t user_id, uint32_t handle)
{
    CWebSocketConn* pMsgConn = NULL;
...
void CImUserManager::GetOnlineUserInfo(list<user_stat_t>* online_user_info)
{
    ...
    for (ImUserMap_t::iterator it = m_im_user_map.begin(); it != m_im_user_map.end(); it++) {
        ...
        if (pImUser->IsValidate()) {
            map<uint32_t, CWebSocketConn*>& ConnMap = pImUser->GetMsgConnMap();
            for (map<uint32_t, CWebSocketConn*>::iterator it = ConnMap.begin(); it != ConnMap.end(); it++)
            {
                CWebSocketConn* pConn = it->second;
//                if (pConn->IsOpen())
//                {
