        ...
        if (pduAttachData.GetPduLength() > 0 && group_cnt > 0)
        {
            ...
            */
            ...
            for (uint32_t i = 0; i < group_info.group_member_list_size(); i++)
            {
                ...
                if (pToImUser && msg2.from_user_id() != member_user_id)
                {
                    //如果接收者在当前服务器
                    Json::Value root;
                    ...
                    {
                        toid=msg2.to_session_id();
                    }

                    message["serviceID"] = SID_MSG;
                    message["commandID"] = CID_MSG_DATA;
                    message["content"] = msg2.msg_data();
                    message["fromId"] = msg2.from_user_id();
                    message["toId"] = toid;
                    message["sessionType"] = msg2.session_type();
                    message["toSessionId"] = msg2.to_session_id();
                    message["msgType"] = msg2.msg_type();
                    message["msgId"] = msg2.msg_id();
                    message["created"] = msg2.create_time();
                    std::string msg_json = writer.write(message);

                    CWebSocketConn *pMsgConn = pToImUser->GetMsgConn(pToImUser->GetHandle());
                    if(pMsgConn && pMsgConn->isHandshark) {
                        pMsgConn->SendMessageToWS(msg_json.c_str());
                    }
...
void CDBServConn::_HandleValidateResponse(CImPdu* pPdu)
{
        ...
        if (result == 0)
        {
            ...
            pUser->SetValidated();
            pUser->SetHandle(pMsgConn->GetConnHandle());

            //给websocket对像加上userid
...
//            }
            ...
            pUser->ValidateMsgConn(pMsgConn->GetConnHandle(), pMsgConn);

            std::string senttext="{\"serviceID\":1,\"commandID\":260,\"code\":1,\"message\":\"验证成功!\"}";
            pMsgConn->SendMessageToWS(senttext.c_str());
        }
        else
        {
            std::string senttext="{\"serviceID\":1,\"commandID\":260,\"code\":0,\"message\":\"密码错误!\"}";
            pMsgConn->SendMessageToWS(senttext.c_str());
        }
 }
...
void CDBServConn::_HandleMsgData(CImPdu *pPdu)
{
    ...
    if(pFromImUser)
    {
        ...
        Json::Value message;
        message["serviceID"] = SID_MSG;
        message["commandID"] = CID_MSG_DATA_ACK;
        message["user_id"] = from_user_id;
        message["sessionType"] = ::IM::BaseDefine::SESSION_TYPE_SINGLE;
        message["to_user_id"] = to_user_id;
        message["msgId"] = msg.msg_id();
        std::string msg_json = writer.write(message);

        CWebSocketConn *pMsgConn = pFromImUser->GetMsgConn(pFromImUser->GetHandle());
        if(pMsgConn && pMsgConn->isHandshark)
        {
            pMsgConn->SendMessageToWS(msg_json.c_str());
        }
    ...
    if (pToImUser) {
        ...
        */
        ...
        {
            toid=msg.to_session_id();
        }

        message["serviceID"] = 3;
        message["commandID"] = 769;
        message["content"] = msg.msg_data();
        message["fromId"] = from_user_id;
        message["toId"] = toid;
        message["sessionType"] = msg.session_type();
        message["toSessionId"] = msg.to_session_id();
        message["msgType"] = msg.msg_type();
        message["msgId"] = msg.msg_id();
        message["created"] = msg.create_time();
        std::string msg_json = writer.write(message);

        log("HandleMsgData, 发送内容。%s",msg_json.c_str());
        if (pToImUser) {
            CWebSocketConn *pMsgConn = pToImUser->GetMsgConn(pToImUser->GetHandle());
            if(pMsgConn && pMsgConn->isHandshark)
            {
...
void CDBServConn::HandleGroupMessage(CImPdu* pPdu) {
    ...
    if (pFromImUser) {
        ...
        message["serviceID"] = SID_MSG;
        message["commandID"] = CID_MSG_DATA_ACK;
        message["to_session_id"] = to_group_id;
        message["user_id"] = from_user_id;
        message["sessionType"] = ::IM::BaseDefine::SESSION_TYPE_SINGLE;
        message["to_user_id"] = from_user_id;
        message["msgId"] = msg.msg_id();
        std::string msg_json = writer.write(message);
